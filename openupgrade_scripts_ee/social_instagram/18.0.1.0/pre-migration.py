...
def _prefill_instagram_message(env):
    """Copy the shared ``message`` field to the new ``instagram_message``
...
def _prefill_instagram_image_ids(env):
    """Create the new Instagram-specific image M2M relation tables and copy
    existing shared image attachments into them.
    """
    openupgrade.logged_query(
        env.cr,
        """
        CREATE TABLE IF NOT EXISTS template_instagram_image_ids_rel (
    ...
    if openupgrade.table_exists(env.cr, "social_post_template_ir_attachment_rel"):
        openupgrade.logged_query(
            env.cr,
            """
            INSERT INTO template_instagram_image_ids_rel
    ...
    openupgrade.logged_query(
        env.cr,
        """
        CREATE TABLE IF NOT EXISTS instagram_image_ids_rel (
    ...
    if openupgrade.table_exists(env.cr, "social_post_ir_attachment_rel"):
        openupgrade.logged_query(
            env.cr,
            """
            INSERT INTO instagram_image_ids_rel
...
def migrate(env, version):
    _prefill_instagram_message(env)
    _prefill_instagram_image_ids(env)

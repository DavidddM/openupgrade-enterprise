...
def _migrate_pe_certificates(env):
    """Migrate l10n_pe_edi.certificate records to certificate.certificate.

    In 17.0: l10n_pe_edi defined its own model l10n_pe_edi.certificate
    (table: l10n_pe_edi_certificate) with PFX content and password.
    ...
    env.cr.execute(
        ...
        SELECT
            old.id,
            COALESCE(old.serial_number, 'PE Certificate'),
            old.password,
            old.company_id,
            att.db_datas,
            old.create_uid, old.create_date, old.write_uid, old.write_date
        FROM l10n_pe_edi_certificate old
        LEFT JOIN ir_attachment att
            ON att.res_model = 'l10n_pe_edi.certificate'
    ...
    if openupgrade.column_exists(
        ...
        openupgrade.logged_query(
            ...
            SET l10n_pe_edi_certificate_id = sub.cert_id
            FROM (
                SELECT DISTINCT ON (company_id)
                    company_id, id AS cert_id
                FROM certificate_certificate
                WHERE company_id IN (
                    SELECT DISTINCT company_id FROM l10n_pe_edi_certificate
                )
                ORDER BY company_id, id DESC
            ) sub
            WHERE rc.id = sub.company_id
              AND (rc.l10n_pe_edi_certificate_id IS NULL
                   OR rc.l10n_pe_edi_certificate_id = 0)
            """,
        )

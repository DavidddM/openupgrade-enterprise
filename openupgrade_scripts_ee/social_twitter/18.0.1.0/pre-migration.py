...
def _prefill_twitter_message(env):
    """Copy the shared ``message`` field to the new ``twitter_message``
    ...
    openupgrade.logged_query(
        ...
        ADD COLUMN IF NOT EXISTS twitter_message text
        """,
    )
    openupgrade.logged_query(
        env.cr,
...
def _prefill_twitter_image_ids(env):
    """Create the new Twitter-specific image M2M relation tables and copy
    existing shared image attachments into them.
    """
    openupgrade.logged_query(
        env.cr,
        """
        CREATE TABLE IF NOT EXISTS template_twitter_image_ids_rel (
    ...
    if openupgrade.table_exists(env.cr, "social_post_template_ir_attachment_rel"):
        openupgrade.logged_query(
            env.cr,
            """
            INSERT INTO template_twitter_image_ids_rel
    ...
    openupgrade.logged_query(
        env.cr,
        """
        CREATE TABLE IF NOT EXISTS twitter_image_ids_rel (
    ...
    if openupgrade.table_exists(env.cr, "social_post_ir_attachment_rel"):
        openupgrade.logged_query(
            env.cr,
            """
            INSERT INTO twitter_image_ids_rel
...
def migrate(env, version):
    _prefill_twitter_message(env)
    _prefill_twitter_image_ids(env)
